...
type Service struct {
	...
	// Dedicated HTTP vhost listener.
	// NOTE(ycfrp): upstream keeps this as a local inside NewService, so the
	// listening socket is never released by Close(). The panel restarts the
	// kernel in-process whenever settings change, so we retain it here and
...
func (svr *Service) Close() error {
	...
	// NOTE(ycfrp): release the HTTP vhost socket as well. When the vhost shares
	// the bind port the listener is owned by the muxer (closed below), so only
	// a dedicated listener needs to be shut down here.
	if svr.httpVhostOwned && svr.httpVhostListener != nil {
		_ = svr.httpVhostListener.Close()
	}
	if svr.sshTunnelGateway != nil {
